...
@method_decorator(
    ratelimit(key="user", rate="10/m", method="GET", block=True), name="dispatch"
)
    ...
    def get(self, request: HttpRequest) -> HttpResponse:
        ...
        if stripe_account.stripe_account_id:
            try:
                ...
                logger.warning(
                    "Failed to retrieve Stripe account for user %s", request.user.id
                )

        if not stripe_account.stripe_account_id:
            try:
                account = services.create_stripe_account(
                ...
                messages.error(
                    request, "Unable to initiate Stripe onboarding. Please try again."
                )
                return redirect(reverse("reimbursements:package-list"))

        refresh_url = request.build_absolute_uri(
            reverse("reimbursements:stripe-onboard")
        )
        ...
        try:
            ...
            logger.exception(
                "Failed to create Stripe account link for user %s", request.user.id
            )
            messages.error(
                request,
